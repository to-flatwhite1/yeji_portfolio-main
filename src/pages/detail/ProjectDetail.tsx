        ...
        <Modal>
          <DetailContainer>
            <DetailWrap imgurl={projectDetail.imgurl}>
              ...
              <DetailBody>
                <ProjectInfoTxt>{projectDetail.subtext}</ProjectInfoTxt>
                ...
                <PWrap>
                  ...
                  <Toggles>
                    {projectDetail.tools.map((item, idx) => (
                      <ToggleBox key={idx} title={item.title} detail={item.detail} type="tools" />
                    ))}
                  </Toggles>
                </PWrap>

                <PWrap>
                  {projectDetail.parts && projectDetail.parts.length > 0 && (
                    <>
                      <PTitle>✨ 작업 기여도</PTitle>
                      <Toggles>
                        {projectDetail.parts.map((item, idx) => (
                          <ToggleBox key={idx} title={item.title} detailList={item.detailList} isToggled />
                        ))}
                      </Toggles>
                    </>
                  )}
                ...
                {projectDetail.detailimginfo && (
                  <PWrap>
                    <PTitle>
                      💻 작업 화면
                      <PSubText>
                        이미지 클릭 시 크게 볼 수 있습니다. (작업화면이 현재와 다를 수 있습니다.)
                        <br />* 저작권 이슈가 있는 경우 첨부하지 않았습니다.
                      </PSubText>
                    </PTitle>
                    ...
                    <ImgContList>
                      {imgModal && (
                        <ImgDetailModal
                          imgUrl={projectDetail.detailimginfo && projectDetail.detailimginfo[targetId].imgurl}
                        />
                      )}
